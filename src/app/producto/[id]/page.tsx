...
export default function PublicProductPage() {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      await navigator.share({ title: product?.nombre, text: settings.empresa.descripcion, url });
    } else {
  ...
  if (!product) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-white px-6 text-center">
        ...
        <p className="mt-2 text-muted-foreground">El enlace que abriste no es válido.</p>
      </div>
    );
  }
        ...
        <header className="mb-8 flex items-center justify-between border-b border-border pb-6">
          <div className="flex items-center gap-4">
            {settings.empresa.logo ? (
              <img src={settings.empresa.logo} alt="" className="h-12 w-auto object-contain" />
            ) : (
        ...
        <div className="grid gap-8 md:grid-cols-2">
          <div>
            <div className="aspect-square w-full overflow-hidden rounded-2xl bg-gray-50">
              <img src={images[currentImage]} alt={product.nombre} className="h-full w-full object-contain p-6" />
            </div>
            {images.length > 1 && (
          ...
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold text-foreground">{product.nombre}</h2>
            ...
            {product.descripcion && (
              <div className="mt-6">
                <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Descripción</h3>
                <p className="mt-2 whitespace-pre-line leading-relaxed text-foreground">{product.descripcion}</p>
              </div>
            )}
