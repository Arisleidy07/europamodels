...
export default function AdminBrands() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await updateBrand(editing.id, { nombre: name.trim() });
    ...
    <div className="space-y-6">
      <div className="flex items-end gap-2">
        <div className="flex-1">
          <label className="mb-1 block text-sm font-medium">Nueva marca</label>
          <Input
            value={newName}
                ...
                <td className="px-4 py-3 text-right">
                  {editing?.id === brand.id ? (
                    ...
                    <div className="flex justify-end gap-2">
                      <button
                        onClick={() => setEditing({ id: brand.id, nombre: brand.nombre })}
